...
def calculate_metrics(pred, gt, lpips_dist, args):
    ...
    for frame_i in range(-args.out_len, 0):
        for batch_i in range(batch_size):
            ...
            ssim_mean[frame_i] += structural_similarity(gt_frame, pred_frame, multichannel=multi_channel, data_range=1.0)/batch_size
        ...
        # send tensors to available device (cuda if available else cpu)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        batch_gt = torch.from_numpy(batch_gt).float().to(device)
