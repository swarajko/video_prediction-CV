...
if __name__ == '__main__':
    ...
    # load checkpoint (map to available device to avoid CUDA-only deserialization errors)
    map_location = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    pred_model.load_state_dict(torch.load(args.checkpoint_load_file, map_location=map_location))
    print('Checkpoint is loaded from ' + args.checkpoint_load_file + ' (mapped to ' + str(map_location) + ')')

    # prepare dataloader for selected dataset
    if args.dataset == 'movingmnist':
        test_dataset = MovingMNIST(args.test_data_dir, seq_len=args.short_len+args.out_len, train=False, img_size=args.img_size)
        ...
        test_dataset = KTH(args.test_data_dir, seq_len=args.short_len+args.out_len, train=False, img_size=args.img_size)
        testloader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, drop_last=False)
